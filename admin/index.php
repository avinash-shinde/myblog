...
if(isset($_GET['delpost'])){ 
    ...
    $stmt = $db->prepare('DELETE FROM blog_post WHERE articleId = :articleId') ;
...
<?php 
    ...
    <?php
        try {
            ...
            $stmt = $db->query('SELECT bp.articleId, bp.articleTitle, bc.catName,bp.cdate FROM blog_post as bp JOIN blog_category as bc ON bp.catId = bc.catId ORDER BY bp.articleId DESC');
            while($row = $stmt->fetch()){
                
                echo '<tr>';
                echo '<td>'.$row['articleTitle'].'</td>';
                echo '<td>'.$row['catName'].'</td>';
                echo '<td>'.date('jS M Y', strtotime($row['cdate'])).'</td>';
                ?>
